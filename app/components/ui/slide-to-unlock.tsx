...
import { type ComponentPropsWithoutRef, createContext, useCallback, useContext, useRef, useState } from 'react'
...
export function SlideToUnlock({ className, handleWidth = 56, children, onUnlock, ...props }: SlideToUnlockRootProps) {
...
export type SlideToUnlockTextProps = Omit<ComponentPropsWithoutRef<typeof motion.div>, 'children'> & {
...
export function SlideToUnlockHandle({ className, children, style, ...props }: SlideToUnlockHandleProps) {
  ...
  return (
    ...
    >
      {children ?? (
        <svg aria-hidden viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
          <path
